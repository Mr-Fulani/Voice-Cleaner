...
#### `max_voice` - Максимальное подавление
Экстремально агрессивные настройки для максимального подавления музыки.

#### `center_voice` - Извлечение центрального канала
Если голос записан в центр стерео, а музыка по бокам:
- Извлекает центральный канал (где обычно голос)
- Применяет речевую эквализацию
- Улучшает разборчивость тихих звуков

#### `harmonic_voice` - Гармонический анализ
Анализ гармонического содержания для различения музыки и речи:
- Подавляет гармонические компоненты музыки
- Усиливает шумовые характеристики речи
- Адаптивная фильтрация на основе спектрального анализа

#### `ultra_clean` - УЛЬТРА очистка
Комбинация всех техник для максимального качества:
- Извлечение центрального канала + гармонический анализ
- Спектральное вычитание + динамическая эквализация
- Множественные highpass фильтры + точная эквализация
...
```bash
chmod +x DOCKER_RUN.sh  # Только первый раз, если нужно
./DOCKER_RUN.sh
```

**Примечание:** Если получаете ошибку "command not found", используйте `./DOCKER_RUN.sh` (с точкой и слэшем в начале).
